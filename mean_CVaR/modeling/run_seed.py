...
exp_params = {
    'mean_cvar': {'tau': [0.05], 'reg_params': [5]},
    'dr_mean_cvar': {'reg_params': [5], 'tau': [0.05], 'rho': [0.05,0.1,0.25]},
    'cond_mean_cvar': {'reg_params': [5], 'tau': [0.05], 'neighbor_quantile': [0.05,0.1,0.25]},
    'dr_winf_cond_mean_cvar': {'reg_params': [5], 'tau': [0.05], 'gamma_quantile': [0.05,0.1,0.25], 'rho_quantile': [0.05,0.1,0.25]},
    'dr_w2_cond_mean_cvar': {'reg_params': [5], 'tau': [0.05],  'epsilon': [0.05, 0.1,0.15], 'rho_div_rho_min':  [1.05,1.1,1.15]},#[1.1,1.2,1.5]},
    #'dr_trim_cond_mean_cvar': {'reg_params': [0.5], 'tau': [0.05],  'gamma': [0.05,0.1,0.25], 'rho_quantile':  [0.2,0.3,0.5]},1.005,1.05,
    'equal_weight': {'reg_params': [5]},
    ...
    #'dr_w2_cond_mean_variance': {'reg_params': [5], 'epsilon': [0.001,0.01, 0.05, 0.1], 'rho_div_rho_min':  [1.005,1.05,1.1,1.2]},
}
import itertools
...
def read_model_sim_ana_summary(model_name, opt_params_dict = None, start_date = 20190101, end_date = 20210101, summarizer = lambda x: x.mean()):
    if opt_params_dict is None:
        opt_params_dict = {
            ...
            'mean_cvar': ['tau','reg_params'],
            'dr_mean_cvar': ['reg_params', 'tau','rho'],
            'cond_mean_cvar': ['reg_params', 'tau','neighbor_quantile'],
            'dr_winf_cond_mean_cvar': ['reg_params','tau', 'gamma_quantile', 'rho_quantile'],
            'dr_w2_cond_mean_cvar': ['reg_params','tau','epsilon', 'rho_div_rho_min'],
        }
...
model_list = ['equal_weight', 'mean_cvar', 'dr_mean_cvar','cond_mean_cvar','dr_winf_cond_mean_cvar','dr_w2_cond_mean_cvar']
...
].loc[['equal_weight', 'mean_cvar', 'dr_mean_cvar','cond_mean_cvar','dr_winf_cond_mean_cvar','dr_w2_cond_mean_cvar']].round(4).to_latex())